-- JEFFLink financial enforcement migration (additive)
...
ALTER TABLE contracts
  ADD COLUMN IF NOT EXISTS installments_generated BOOLEAN NOT NULL DEFAULT FALSE;
...
CREATE UNIQUE INDEX CONCURRENTLY IF NOT EXISTS idx_installments_contract_due
  ON installments(contract_id, due_date);
...
ALTER TABLE payments
  ADD COLUMN IF NOT EXISTS payment_type VARCHAR(30) NOT NULL DEFAULT 'INSTALLMENT'
...
ALTER TABLE payments
  ADD COLUMN IF NOT EXISTS recovery_case_id UUID REFERENCES recovery_cases(id);
...
CREATE UNIQUE INDEX CONCURRENTLY IF NOT EXISTS idx_payments_momo_transaction_id
...
ALTER TABLE ledger_entries
  ADD CONSTRAINT IF NOT EXISTS ledger_entries_transaction_id_not_null
  CHECK (transaction_id IS NOT NULL) NOT VALID;
...
FOR EACH ROW
EXECUTE PROCEDURE prevent_ledger_mutation();

CREATE OR REPLACE FUNCTION prevent_ledger_transaction_mutation() RETURNS trigger AS $$
BEGIN
...
FOR EACH ROW
EXECUTE PROCEDURE prevent_ledger_transaction_mutation();
...
CREATE OR REPLACE FUNCTION ensure_payment_has_ledger() RETURNS trigger AS $$
DECLARE
  v_count INT;
BEGIN
  IF NEW.status = 'SUCCESS' AND OLD.status IS DISTINCT FROM NEW.status THEN
    ...
    WHERE le.transaction_id IN (
      SELECT id FROM ledger_transactions WHERE payment_id = NEW.id
    );

    IF v_count = 0 THEN
      RAISE EXCEPTION 'Payment % cannot be SUCCESS without ledger entries', NEW.id;
    END IF;
  END IF;
...
DROP TRIGGER IF EXISTS trg_payment_requires_ledger ON payments;
CREATE TRIGGER trg_payment_requires_ledger
BEFORE UPDATE OF status ON payments
FOR EACH ROW
EXECUTE PROCEDURE ensure_payment_has_ledger();
...
BEGIN
  ...
  v_grace_days := COALESCE(v_config.grace_days, p_grace_days);
  v_default_days := COALESCE(p_default_days, 30);
  ...
  FOR rec IN
    WITH candidates AS (
      SELECT id,
             ...
             CASE
               WHEN v_config.id IS NULL THEN penalty_amount
               WHEN v_config.penalty_type = 'FLAT' THEN
                 ROUND(GREATEST((v_today - due_date)::INT, 0) * v_config.penalty_rate, 2)
               WHEN v_config.penalty_type = 'PERCENT' THEN
                 ROUND(
                   GREATEST((v_today - due_date)::INT, 0)
                   * (amount_due * v_config.penalty_rate / 100),
                   2
                 )
               ELSE penalty_amount
             END AS new_penalty
  ...
  WHERE status IN ('ACTIVE', 'OVERDUE', 'DEFAULT_WARNING')
    AND EXISTS (
      SELECT 1
      FROM installments i
      WHERE i.contract_id = contracts.id
...
-- Vendor withdrawals
ALTER TABLE wallets
  ADD CONSTRAINT IF NOT EXISTS wallets_balance_non_negative CHECK (balance >= 0);
...
-- Indexes
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_contract_status ON contracts(status);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_installment_due_date ON installments(due_date);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_installment_contract ON installments(contract_id);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_payment_status ON payments(status);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_vehicle_status ON vehicles(status);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_users_branch_id ON users(branch_id);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_contracts_branch_id ON contracts(branch_id);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_payments_branch_id ON payments(branch_id);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_ledger_transactions_branch_id ON ledger_transactions(branch_id);
CREATE UNIQUE INDEX CONCURRENTLY IF NOT EXISTS idx_ledger_transactions_payment_id
