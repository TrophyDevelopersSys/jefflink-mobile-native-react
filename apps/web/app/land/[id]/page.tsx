...
async function getLandListing(id: string): Promise<ListingDetail | null> {
  try {
...
export async function generateMetadata({
  ...
  const listing = await getLandListing(id);
  if (!listing) return { title: "Property Not Found" };
  return {
    title: listing.title,
    description:
      listing.description ??
      `${listing.title} for sale in ${listing.location}. Price: ${listing.price}`,
    openGraph: {
    ...
    <main className="min-h-screen bg-background">
      <div className="max-w-4xl mx-auto px-4 py-12">
        ...
        <nav className="text-sm text-text-muted mb-6">
          ...
          {" / "}
          <Link href="/land" className="hover:text-white">Land</Link>
          {" / "}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="flex flex-col gap-4">
            ...
            {listing.description && (
              <p className="text-text-muted text-sm leading-relaxed">{listing.description}</p>
            )}
