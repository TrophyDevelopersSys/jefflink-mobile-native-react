...
async function getRelated(excludeId: string): Promise<ListingSummary[]> {
  try {
    const res = await fetch(`${API}/listings?type=property&subtype=land&limit=5`, { next: { revalidate: 60 } });
    if (!res.ok) return [];
    const json = await res.json();
...
export async function generateMetadata({
  ...
  return {
    ...
    description: listing.description ?? `${listing.title} for sale in ${listing.location}. Price: ${listing.price}`,
